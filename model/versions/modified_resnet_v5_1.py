...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, image_size=224, width=64):
        ...
        self.msc_modules = nn.ModuleList([
            MSC_Modified(dim=dims[i]) for i in range(4)
        ])

        self.cov1 = nn.Conv2d(dims[0], dims[-1], 3, 2, 1)
        self.cov2 = nn.Conv2d(dims[1], dims[-1], 3, 2, 1)
        self.cov3 = nn.Conv2d(dims[2], dims[-1], 3, 2, 1)
        self.cov4 = nn.Conv2d(dims[3], dims[-1], 1)
    ...
    def forward(self, x):
        ...
        x1 = self.msc_modules[0](x4, x1)
        x2 = self.msc_modules[1](x4, x2)
        x3 = self.msc_modules[2](x4, x3)
        x4 = self.msc_modules[3](x4, x4)
