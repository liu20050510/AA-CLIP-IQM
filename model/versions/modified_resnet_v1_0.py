...
class MultiScaleConvModule(nn.Module):
    ...
    def forward(self, x):
        outputs = []
        for fc in self.fc_11:
            outputs.append(fc(x))
        for fc in self.fc_33:
            outputs.append(fc(x))
        for fc in self.fc_55:
            outputs.append(fc(x))
        for fc in self.fc_77:
            outputs.append(fc(x))
        for fc in self.fc_51:
            outputs.append(fc(x))
        for fc in self.fc_15:
            outputs.append(fc(x))
        combined_output = torch.sum(torch.stack(outputs), dim=0)
...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, image_size=224, width=64):
        ...
        # 添加多尺度卷积核模块
        self.multi_scale_conv1 = MultiScaleConvModule(width, width)
        self.multi_scale_conv2 = MultiScaleConvModule(width * 2, width * 2)
        self.multi_scale_conv3 = MultiScaleConvModule(width * 4, width * 4)
        self.multi_scale_conv4 = MultiScaleConvModule(width * 8, width * 8)
